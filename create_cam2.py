...
def get_args():
	...
	parse.add_argument('--dataset', type=str, default='CIFAR', choices=['STL', 'CIFAR', 'OWN'])
	parse.add_argument('--dataset_path', type=str, default='./data')
	...
	parse.add_argument('--num_result', type=int, default=1)
...
def create_cam():
    ...
    test_loader, num_class = utils.get_test_loader(
                                                  config.dataset_path,
                                                  config.dataset,
                                                  config.img_size)
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    cnn = model.CNN(img_size=config.img_size, num_classes=num_class).to(device)
    cnn.load_state_dict(
